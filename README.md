...
- `tools/ideal-obj-render-warp-lab` は、FaceBuilder + Blender sculpt 由来の neutral OBJ を current yaw / pitch / roll で render し、その rendered ideal image を MediaPipe に再入力して `renderedIdeal478` を取得する debug / research lab 候補です。`ideal-reference-mesh-warp-lab` の coordinate lifecycle、dynamic grid、anchors、triangle indices、WebGL mesh warp preview を踏襲し、ideal source だけを OBJ render -> MediaPipe returned 478 に差し替えて検証します。MP4 の同一 canvas frame を `detect()`（IMAGE mode / 静止画モード）と `detectForVideo()`（VIDEO mode / 動画モード）の両方へ渡し、pose / 478 landmarks / 実行時間の系統差を見るモード比較も扱います。モード比較の詳細確認、JSON / CSV download、重要フレーム preview export は右ペイン Debug（デバッグ）の `モード比較` タブで扱います。production 方式確定ではなく、Runtime / Studio / Authoring Tool 本線には接続しません。詳細は [Ideal OBJ Render Warp Lab](docs/ideal-obj-render-warp-lab.md) を参照してください。
- `tools/shape-warp-pipeline-lab` は、肥大化した Ideal OBJ Render Warp Lab から shape warp pipeline の確認だけを切り分ける debug / research lab です。current478、renderedIdeal478、alignedRenderedIdeal478、actual visible landmarks、background grid、combined vertices、triangleIndices の流れに絞り、初期版では WebGL mesh warp を実装しません。詳細は [Shape Warp Pipeline Lab](docs/shape-warp-pipeline-lab.md) を参照してください。
...
tools/shape-warp-pipeline-lab
  OBJ render 後段の shape warp pipeline を、preview / debug / download に分離して検証する debug / research lab
...
- Ideal Reference Mesh Warp Lab 3ペインUI / 動画 preview / モデル動画 MediaPipe scan / ライブ current scan / current478 overlay / top1 matching
...
- Shape Warp Pipeline Lab は、Ideal OBJ Render Warp Lab から current face / rendered ideal / alignment / background grid / combined mesh debug だけを分離した lab です。Placement Function Analysis、benchmark、WebGL mesh warp preview は含めません。
...
- [Shape Warp Pipeline Lab](docs/shape-warp-pipeline-lab.md)
