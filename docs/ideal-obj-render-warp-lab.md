...
currentMatrix
  ...
  -> renderedIdeal478 を MediaPipe image-normalized 0..1 座標として clone
  -> normalizedIdeal478
...
placement function の適用対象は、MediaPipe image-normalized coordinate（0..1）の `renderedIdeal478` を同じ 0..1 座標系として clone した `normalizedIdeal478` であり、OBJ coordinate、render canvas pixel coordinate、displayedContentRect pixel coordinate、aspect-corrected work coordinate はここに混ぜない。

```text
normalizedIdeal.x = renderedIdeal.x
...
- `ライブ座標（live image-normalized座標）`: live video image-normalized coordinate（ライブ映像の画像正規化座標）で `current478`、`alignedRenderedIdeal478`、`meshSourceVertices` を確認する。`alignedRenderedIdeal478` は `レンダー画像（render canvas座標）` tab の `renderedIdeal478` を MediaPipe image-normalized coordinate（0..1）の中間値 `normalizedIdeal478` として扱い、placement function 適用後に live video image-normalized coordinate へ入った点である。未配置の `renderedIdeal478` / `normalizedIdeal478` はこの tab には直接描画しない。
...
`renderedIdeal478`（レンダー理想478点）は render canvas image-normalized coordinate（0..1）の点であり、live coordinate tab（ライブ座標タブ）には直接表示しない。live video image-normalized coordinate として表示してよい理想点は、`normalizedIdeal478` に placement function（配置関数）を適用した `alignedRenderedIdeal478` だけとする。
...
`renderedIdeal478`（レンダー理想478点）は render canvas image-normalized coordinate（0..1）の点であり、live video（ライブ映像）上に配置済みの点ではない。`ライブ座標（live image-normalized座標）` tab に表示する理想点は、placement function 適用後の `alignedRenderedIdeal478` である。
...
`ライブ座標（live image-normalized座標）` では `current478` / `alignedRenderedIdeal478` / `meshSourceVertices` を確認します。
...
renderedIdeal478:
  ...
  -> MediaPipe image-normalized 0..1 coordinate として clone
  -> normalizedIdeal478
...
live overlay / mesh target 入力へ戻します。live coordinate tab（ライブ座標タブ）には placement function 適用後の `alignedRenderedIdeal478` を描画します。未配置の `normalizedIdeal478` は render canvas 側の中間値なので、この tab には直接描画しません。
...
pixel coordinate、OBJ vertex coordinate、WebGL clip space は、MediaPipe returned landmarks 取得後の alignment / mesh pair 処理には混ぜません。render image の pixel coordinate は MediaPipe 入力用に閉じ込めます。MediaPipe から戻ってきた `renderedIdeal478` は render canvas image-normalized coordinate（0..1）として扱い、live coordinate tab とライブ映像上には placement function（配置関数）適用後の `alignedRenderedIdeal478` だけを表示します。
