...
- `回転中心評価・粗探索` ボタンは、`fitting_lab_12pt_rotation_center`（Fitting Lab 12点回転中心探索）を searchMode（探索モード）として使います。
- Fitting Lab から採用するのは coordinate descent（座標降下探索）の手順だけです。Fitting Lab の x/y 座標系、`rotationCenter.y/z` range（回転中心探索範囲）、12点 z range（12点奥行き探索範囲）は採用しません。
...
- `zSymmetryMode: paired_left_right` として、`cheek.z` / `eye.z` / `jaw.z` は左右ペアに同じ z を入れて探索します。最終出力は従来通り 12点分の `zByPointId` に展開し、`leftCheek` / `rightCheek`、`leftEye` / `rightEye`、`leftJaw` / `rightJaw` を保持します。
- 粗探索（step `0.01`）の best candidate（最良候補）を base candidate（基準候補）にし、その周辺だけ fine search（細かい追加探索）を行います。fine search は step `0.005`、radius `0.01`、iteration `1` とし、全範囲を細かく探索しません。
- `fitting_lab_12pt_rotation_center`（Fitting Lab 12点回転中心探索）は、debug UI（検証用 UI）として `passMode: two_pass_base_rectification` の 2週構成にできます。1週目は従来どおり frontCandidate（正面候補）の `adjusted12pt` をそのまま base12pt（基準12点）にし、2週目は 1週目の `bestCandidate`（最良候補）を使って基準フレームの yaw（左右向き） / pitch（上下向き） / roll（傾き）を打ち消し、正面へ戻した base12pt の x/y で再探索します。
- 2週目で変えるのは base12pt の x/y だけです。evaluation frame（評価フレーム）の target（比較対象）は従来どおり `x = adjusted12pt.x * videoAspectRatio`、`y = adjusted12pt.y` を使い、pixel coordinate（ピクセル座標）、face bounds center（顔外枠中心）、顔幅 normalization（正規化）は使いません。
- 2週目の目的は、「正面候補が完全な正面ではないこと」が score（誤差スコア）の主因かを見ることです。2週目の結果を無条件に production export（本番書き出し）へ採用するものではありません。
...
- 現在の `回転中心評価・粗探索` ボタンは、旧 Stage A（rotationCenter.y/z coarse search: 回転中心y/z粗探索）と Stage B（group z search: グループ単位奥行き探索）ではなく、Fitting Lab の `12pt_rotation_center`（回転中心推定向け12点）で確認した coordinate descent（座標降下探索）の手順を主導線にする。
- `searchMode`（探索モード）は `fitting_lab_12pt_rotation_center` とする。
- 候補生成は Fitting Lab の `coordinateDescent`（座標降下探索）の実行手順に合わせる。1つの parameter（探索対象）だけを動かして候補を評価し、その step（手順）で最良だった candidate（候補）を次の parameter の base（基準）にする。
- base candidate（基準候補）は Render Consistency Lab 用の `renderUniformDebugInitial`（Render検証用の統一寄り初期値）を使う。`rotationCenter.x`（回転中心x）は `0.5 * videoAspectRatio` 固定、`rotationCenter.y = 0`、`rotationCenter.z = 0.06`、12点 z はすべて `0` とする。
...
- 探索順は `rotationCenter.y` / `rotationCenter.z` の後、左右ペアをまとめた `cheek.z` / `nose.z` / `mouth.z` / `eye.z` / `headTop.z` / `chin.z` / `noseBridge.z` / `jaw.z` / `upperFaceCenter.z` とする。
- `cheek.z` は `leftCheek.z` / `rightCheek.z`、`eye.z` は `leftEye.z` / `rightEye.z`、`jaw.z` は `leftJaw.z` / `rightJaw.z` に同じ値として反映する。探索対象はまとめるが、`bestCandidate.zByPointId` と `finalZByPointId` は従来通り 12点分を出す。
- iterationCount（反復回数）は coarse search（粗探索）では Fitting Lab の preset と同じく `2` とする。fine search（細かい追加探索）は coarse search の best candidate 周辺だけを `step 0.005` / `radius 0.01` / `iteration 1` で追加探索する。
- `passMode: two_pass_base_rectification` では 2週構成にする。firstPass（1週目）は従来どおり正面候補をそのまま base12pt として使い、secondPass（2週目）は firstPass の `bestCandidate.zByPointId` と `rotationCenter` を使って基準フレームの yaw / pitch / roll を逆回転し、正面へ戻した base12pt の x/y で同じ 12+2点探索をもう一度行う。
- secondPass（2週目）の初期候補は firstPass（1週目）の `bestCandidate` とするが、2週目の中で `rotationCenter.y` / `rotationCenter.z` と 12点 z は再探索する。evaluation frame 側の target は変えず、Render Consistency Lab 用の比較座標と探索範囲を使い続ける。
- Raw JSON（生デバッグJSON）の `fittingLab12ptSearch` には `firstPass` / `secondPass` / `passImprovement` / `baseFrameRectification` を残す。top-level の `bestCandidate`、`bestRotationCenter`、`finalZByPointId` は secondPass（2週目）の結果を指してよい。
- Raw JSON（生デバッグJSON）には `fittingLab12ptSearch`、`searchStages.coarse` / `searchStages.fine`、`zSymmetryMode`、`symmetricZParameters`、`leftRightZSymmetryDiagnostics`、`coordinateDescentLog`（座標降下探索ログ）、`parameterImprovements`（探索対象ごとの改善量）、`parameterImprovementSummary`（改善量要約）、`bestCandidate`、`finalZByPointId`（最終的な点ごとの奥行き）、`bestRotationCenter`（最良回転中心）を出す。
- top-level の `parameterImprovements` / `parameterImprovementSummary` は final stage（最終段階）の fine search（細かい追加探索）を指す。coarse search（粗探索）の改善量は `searchStages.coarse.parameterImprovements` / `searchStages.coarse.parameterImprovementSummary` に残す。`scoreDelta` は `scoreAfter - scoreBefore` なので、改善した step（手順）では negative value（負の値）になる。
...
- Fitting Lab から採用するのは `coordinateDescent`（座標降下探索）という探索アルゴリズムと実行手順だけであり、Fitting Lab の座標系・探索範囲・12点 z range（12点奥行き探索範囲）は採用しない。
...
- 12点 z range（12点奥行き探索範囲）は Render Consistency Lab 用に再定義する。Render 用の uniform-ish debug range（統一寄りの検証用範囲）であり、production asset（本番用アセット）ではなく coordinate descent（座標降下探索）の挙動確認用である。
...
- `coordinateBoundaryStatus`（座標降下探索の範囲端ヒット状態）で、Render 用探索範囲でも `rotationCenter.y` が `0.50` 上限に張り付くか、`rotationCenter.z` や 12点 z が範囲端に張り付くか確認する。
- Raw JSON（生デバッグJSON）の `fittingLab12ptSearch` には `coordinateSystemSource: render_adjusted12pt_aspect_corrected`、`rangeSource: render_consistency_lab`、`zRangeSource: render_consistency_lab_uniform_debug_range`、`fittingLabAlgorithmOnly: true` を出す。
