...
- Rotation Fit（回転中心評価）タブには `Improvement by parameter（探索対象ごとの改善量）` を coarse search（粗探索）と fine search（細かい追加探索）に分けて表示し、coordinate descent（座標降下探索）の各 step（手順）で `parameter`（探索対象）、`valueBefore` / `valueAfter`、`scoreBefore` / `scoreAfter`、`improvement`（改善量）、`maxFrameScoreImprovement`（最大フレームスコア改善量）を確認できるようにします。
