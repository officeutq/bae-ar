...
- 初期結果で `bestRotationCenter.y`（最良の回転中心y）が探索範囲上限に張り付いたため、`rotationCenter.y`（回転中心の縦方向）の探索範囲は `-0.24 .. 0.16 / step 0.02` に広げています。`rotationCenter.z`（回転中心の奥行き方向）は `0.00 .. 0.12 / step 0.01` のままです。
