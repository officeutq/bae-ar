...
現在 `tools/ideal-face-authoring` は Step 2-E まで実装済みです。`tools/layer-mask-authoring` は将来予定です。
...
Engine Runtime は動画入力、フレーム抽出、代表フレーム抽出、手動ラベル確定、3D推測用 dataset 作成、`idealLandmarks3D` 作成を行いません。Runtime は完成済みの IdealFace asset を読み込み、`idealLandmarks3D` 478点を現在 `FacePose` へ投影して projected ideal 2D landmarks 478点を生成します。
...
- IdealFace Authoring Tool は Step 2-E まで実装済み
