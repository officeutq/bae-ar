...
Step R: Ideal Reference Mesh Warp Lab
  ...
  現在はモデル動画の MediaPipe 解析、raw ideal reference frames 作成、ライブ current 解析、top1 matching、current -> aligned ideal の displacement debug まで実装済み
  ...
  topK weighted blend、visibilityWeight / warpSafetyWeight、hybrid mesh、mesh warp は後段で扱う
```
