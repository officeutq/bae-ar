...
Step R: Ideal Reference Mesh Warp Lab
  ...
  現在はモデル動画の MediaPipe 解析、raw ideal reference frames 作成、ライブ current 解析、top1 matching、current -> aligned ideal の displacement debug、調整なし raw displacement mesh warp prototype、warp coordinate debug、既存座標系方針に従う raw warp 表示実験まで実装済み
