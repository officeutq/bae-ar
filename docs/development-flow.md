...
expression-aware attenuation は、目だけ大きくする、鼻だけ細くする、顎だけ削るための機能ではありません。口を開けたときの `mouth` group、まばたき / 目細め時の `left_eye` / `right_eye` group、顔外周の `face_boundary` group など、破綻しやすい領域の補正を弱める safety attenuation として扱います。
