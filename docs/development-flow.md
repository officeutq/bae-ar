...
Step 2-I-A では UI / state 基盤を実装済みです。Step 2-I-B では pose-aware multi-frame inference dataset 作成を実装済みです。Step 2-I-C では pose-aware weighted z inference v1 の 3D候補生成を追加しました。Step 2-G は現在実装済みの旧簡易推定として残し、Step 2-I 用操作は Step 2-I カード内に閉じ、旧ポーズ別候補 UI には混ぜません。画面上の 3 分類は排他的に表示します。
...
Step 2-I-B では、pose-aware multi-frame inference dataset 作成に範囲を絞っています。`frontReferenceFrames` は複数の正面基準候補から base x / y を作る入力として作り、`observationFrames` は除外されていない解析成功フレームから作ります。observation は frameId、timestamp、2D landmarks、FacePose yaw / pitch / roll、score、poseStrength、weight を持ち、`left / right / up / down` 固定分類は持ちません。Step 2-I-C ではこの dataset を使い、Step 2-G v1 を置き換えず別方式の candidate として pose-aware weighted z inference v1 を実装しています。
