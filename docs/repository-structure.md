...
## Ideal OBJ Render Warp Lab の座標方針

`tools/ideal-obj-render-warp-lab` は、`ideal-reference-mesh-warp-lab` の座標系・メッシュ・WebGL warp を踏襲し、ideal source だけを OBJ render -> MediaPipe returned 478 に差し替える将来予定の debug / research lab です。
...
`renderedIdeal478` は MediaPipe returned landmarks として image-normalized coordinate で扱います。alignment の bounds / center / uniform scale / distance / large displacement 判定では、既存ラボと同じ aspect-corrected image coordinate を使います。
...
この tool では現時点で mesh pair、triangle wireframe、WebGL mesh warp input debug、Lab 内限定の WebGL mesh warp preview prototype の overlay / summary / preview raw debug と、model scan JSON export / import を扱います。triangle indices は source vertices の位置を基準に作り、source mesh と target mesh で共通に使います。triangle quality は aspect-corrected image coordinate で評価します。model scan JSON は `rawIdealReferenceFrames` と accepted / excluded frame 管理を再利用するためのもので、model video 本体、live video 本体、current mesh、triangle indices、WebGL runtime state は含めません。production mesh warp、raw displacement mesh warp、Runtime / Engine 変更、Beauty Studio 変更、IdealFace Authoring Tool 変更、IndexedDB / localStorage 保存、compression、validator package 化は含めません。
...
この段階では Lab 内限定 preview として WebGL context 初期化、texture upload、shader、`gl.drawElements` までを扱います。ただし production mesh warp は含めません。texture flip 実験 UI、`rawWarpOnly`、`sideBySide`、raw displacement mesh warp も復活させません。
...
この lab 段階では production renderer 化、Runtime / Engine 変更、Beauty Studio 変更、IdealFace Authoring Tool 変更は含めない。texture flip 実験 UI、raw displacement mesh warp、`showRawWarp`、`rawWarpOnly`、`sideBySide` は復活させない。temporal smoothing、topK reference blend、production renderer lifecycle、performance hardening もまだ扱わない。
