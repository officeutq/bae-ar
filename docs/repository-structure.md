...
この tool では現時点で mesh pair、triangle wireframe、WebGL mesh warp input debug、Lab 内限定の WebGL mesh warp preview prototype の overlay / summary / preview raw debug を扱います。triangle indices は source vertices の位置を基準に作り、source mesh と target mesh で共通に使います。triangle quality は aspect-corrected image coordinate で評価します。production mesh warp、raw displacement mesh warp、Runtime / Engine 変更、Beauty Studio 変更、IdealFace Authoring Tool 変更、JSON export / import、保存機能は含めません。
