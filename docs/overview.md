...
理想モデル動画の実測 MediaPipe 478 reference library を使う新しい mesh warp 検証は [Ideal Reference Mesh Warp Lab](ideal-reference-mesh-warp-lab.md) に整理します。`tools/ideal-reference-mesh-warp-lab` は3ペインUI、モデル動画 / ライブ動画 preview、モデル動画の MediaPipe 解析と raw ideal reference frames 作成、accepted frame review、モデル動画タブでの 478点 overlay 表示まで実装済みです。ライブ動画の MediaPipe 解析、reference matching、visibilityWeight / warpSafetyWeight、hybrid mesh、mesh warp、Runtime renderer integration、JSON export、IndexedDB、compression、validator は未実装です。
